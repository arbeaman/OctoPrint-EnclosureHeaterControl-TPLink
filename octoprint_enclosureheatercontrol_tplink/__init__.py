...
# Sub-plugin that switches an enclosure heater through a TP-Link Kasa smart plug over the local
# network, using TP-Link's legacy protocol on TCP port 9999. Registers with Enclosure Heater
# Control as a switching and sensing method.
...
class EnclosureHeaterControl_TPLink(octoprint.plugin.StartupPlugin,
                        octoprint.plugin.RestartNeedingPlugin,
                        octoprint.plugin.TemplatePlugin,
                        octoprint.plugin.SettingsPlugin):
    """Sub-plugin that drives an enclosure heater through a TP-Link Kasa smart plug."""
    ...
    def change_enclosure_heater_control_state(self, state):
    ...
    def turn_enclosure_heater_control_on(self):
        """Switch the plug on."""
        self._logger.debug("Switching Enclosure Heater On")
        self.change_enclosure_heater_control_state(1)


    def turn_enclosure_heater_control_off(self):
        """Switch the plug off."""
        self._logger.debug("Switching Enclosure Heater Off")
        self.change_enclosure_heater_control_state(0)


    def get_enclosure_heater_control_state(self):
        """Return whether the plug, or the selected outlet, is currently on."""
        self._logger.debug("get_enclosure_heater_control_state")
        sysinfo = self.get_sysinfo()
    ...
    def get_update_information(self):
        ...
        return dict(
            enclosureheatercontrol_tplink=dict(
                displayName="Enclosure Heater Control - TPLink",
                displayVersion=self._plugin_version,

                # version check: github repository
                type="github_release",
                user="arbeaman",
                repo="OctoPrint-EnclosureHeaterControl-TPLink",
                current=self._plugin_version,

                # update method: pip w/ dependency links
                pip="https://github.com/arbeaman/OctoPrint-EnclosureHeaterControl-TPLink/archive/{target_version}.zip"
            )
        )

__plugin_name__ = "Enclosure Heater Control - TPLink"
__plugin_pythoncompat__ = ">=3,<4"

def __plugin_load__():
    """Instantiate the sub-plugin and register its Software Update hook."""
    global __plugin_implementation__
    __plugin_implementation__ = EnclosureHeaterControl_TPLink()
